      ...
      if (editProducts.length > 0) {
        ...
        if (firstProduct && (firstProduct as any).customColumns && Array.isArray((firstProduct as any).customColumns)) {
          ...
          const backendCustomCols = (firstProduct as any).customColumns.map((col: any) => ({
            ...
            label: col.label || col.key.replace(/^custom_/, '').replace(/_/g, ' ').replace(/\b\w/g, (l:any) => l.toUpperCase()),
            width: col.width || 150,
          }));
        ...
        // For new products, extract from row keys
        const allCustomKeys = new Set<string>();
...
useEffect(() => {
  ...
  const handleFinalSubmit = async () => {
    try {
      ...
      // Helper to map vendor code to enum value
      const mapVendorCodeToEnum = (code: string | null | undefined): string | null => {
        if (!code) return null;
        const codeLower = String(code).trim().toLowerCase();
        // Map dropdown codes to enum values - backend enum: ['att', 'tmob']
        const vendorEnumMap: Record<string, string> = {
          '1': 'att',
          '2': 'tmob',
          'att': 'att',
          'tmob': 'tmob',
        ...
        return vendorEnumMap[codeLower] || normalizeEnum(code, ['att', 'tmob']);
      };
      ...
      const productsToCreate = calculationResults.map((result) => {
        ...
        return {
          ...
          vendor: mapVendorCodeToEnum(row.vendor),
      ...
      if (editProducts && editProducts.length > 0) {
        ...
        onSave(updatedRows, variantType === 'multi' ? pendingTotalMoq : undefined, customColumns);
  ...
  const columns: ColumnType[] = [
    ...
    { key: 'grade', label: 'GRADE*', width: 140, group: 'Product Detail' },
    ...
    { key: 'tags', label: 'TAGS', width: 190, group: 'Other Info' },
      ...
      case 'endTime':
        ...
        return (
          <DatePicker
            ...
            maxDate={column.key === 'startTime' ? maxDateForStartTime : undefined}
            className="w-full px-2 py-1.5 text-xs border-0 bg-transparent focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 rounded transition-all duration-150 placeholder:text-gray-400"
